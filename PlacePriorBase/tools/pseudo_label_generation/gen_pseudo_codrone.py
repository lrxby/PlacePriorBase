...
# best parameters of row 5 (CODrone)
...
def analyze_geometry_specific(mask_stage1, gt_points, default_sigma, device, method):
    """CODrone geometry analysis: Rect (minAreaRect)."""
    J = len(gt_points)
    ...
    for j in range(J):
        ...
        if method == 'Row6_Rect':
            ...
            # normalize the OpenCV angle to keep the long edge
...
def generate_codrone():
    ...
    for file_id in tqdm.tqdm(files):
        ...
        for idx in range(J):
            ...
            # anchor strictly at the GT point for accuracy
