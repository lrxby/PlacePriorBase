...
# Best parameters of the current method

CFG = dict(sigma=4096, ds=2, pos=0.996, neg=0.009)  # Rect

# post-processing (applied directly)
POST_PROCESS = {
    11: 1.2  # roundabout scaled by 1.2x
}
...
def analyze_geometry_specific(mask_stage1, gt_points, default_sigma, device, method):
    """General geometry analysis with Rect (minAreaRect)."""
    J = len(gt_points)
    sigmas_list = []
    angles_list = []
    ...
    for j in range(J):
        ...
        # --- extract the angle with the selected method ---
        if method == 'Row6_Rect':
            rect = cv2.minAreaRect(mask_pts)
            _, _, angle_deg_rect = rect
            # align to the physical long edge
        ...
        # sigma for stage 2 (strictly anchored at the GT center)
...
def generate_pseudo_labels():
    ...
    for i, file_id in enumerate(tqdm.tqdm(files)):
        ...
        # 3. geometry analysis (with the selected method)
        ...
        # use the most robust np.where extraction
        for idx in range(J):
            ...
            # fallback to a small default box so the detector does not miss the point
            if len(px) < 3:
            ...
            # anchor strictly at the annotated GT point, avoiding center drift
            ...
            # saved format: [x, y, w, h, angle, label]
