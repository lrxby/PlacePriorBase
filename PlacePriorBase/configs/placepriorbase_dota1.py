...
model = dict(
    ...
    bbox_head=dict(
        ...
        loss_ourwater=dict(type='VWRBoxLoss', topk=0.90, loss_weight=0.5, scale_factor=6.0),

        loss_size=dict(type='SizePriorLoss', loss_type='l2', loss_weight=0.1, scale_factor=10.0, topk=0.90,
            target_classes=[0, 1, 4, 7, 8, 9, 10, 11, 14]),

        loss_angle=dict(type='AnglePriorLoss', eps=0.01, topk=0.90, loss_weight=0.2, scale_factor=5.0,
            k_radius=2.0,
            score_alpha=1.0,
...
train_pipeline = [
    ...
    dict(type='LoadPseudoAnnotations',
         pkl_path='data/split_ss_dota/trainval/dota1-rect.pkl'),
