...
model = dict(
    ...
    bbox_head=dict(
        ...
        num_classes=5,          # [DroneVehicle] 5 classes
        in_channels=128,
        feat_channels=128,
        strides=[8],
...
# ---------------------------------------------------------------
# train pipeline (DroneVehicle 800x800):
# LoadPseudoAnnotations -> ConvertBoxType -> ConvertWeakSupervision
#   -> Resize(800,800) -> RandomFlip -> PseudoBoxSync -> PackDetInputs
# RandomRotate / RBox2PointWithNoise are removed; only random flip is kept
# ---------------------------------------------------------------
train_pipeline = [
    dict(type='mmdet.LoadImageFromFile', backend_args={{_base_.backend_args}}),
    dict(type='mmdet.LoadAnnotations', with_bbox=True, box_type='qbox'),
    dict(type='LoadPseudoAnnotations',
         pkl_path='data/split_ss_dronevehicle/trainval/dronevehicle-Rect.pkl'),
...
# ---------------------------------------------------------------
# test annotations: ann_file='test/annfiles'
# ---------------------------------------------------------------
# val_pipeline (defined explicitly here)
val_pipeline = [
