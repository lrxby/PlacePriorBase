...
class CODRONEDataset(BaseDataset):
    ...
    def load_data_list(self) -> List[dict]:
        """Load annotations from an annotation file named as ``self.ann_file``"""
        # string -> ID mapping (e.g., 'boat': 10)
        cls_map = {c: i for i, c in enumerate(self.metainfo['classes'])}
        data_list = []

        # case 1: empty ann_file (usually test mode)
        if self.ann_file == '':
            img_files = glob.glob(
                osp.join(self.data_prefix['img_path'], f'*.{self.img_suffix}'))
            for img_path in img_files:
                ...
                img_name = osp.split(img_path)[1]
                data_info['file_name'] = img_name
                img_id = img_name[:-len(self.img_suffix)-1]  # strip suffix and dot
                data_info['img_id'] = img_id
        ...
        # case 2: standard TXT annotation files
        else:
            txt_files = glob.glob(osp.join(self.ann_file, '*.txt'))
            if len(txt_files) == 0:
            ...
            for txt_file in txt_files:
                ...
                with open(txt_file, 'r') as f:
                    ...
                    for line in lines:
                        ...
                        # 1. format check: at least 9 columns (8 coords + 1 class)
                        # the standard format has 10 columns (with difficulty); keep difficulty optional
                        if len(parts) < 9:
                            continue
                        ...
                        # parse difficulty (10th column, if present)
                        if len(parts) > 9:
                            try:
                ...
                # keep images with valid data (or in training mode)
                # empty images with empty instances are handled by MMDet
                data_info['instances'] = instances
                data_list.append(data_info)
